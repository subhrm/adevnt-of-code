...
The first 6 occurs after only two changes, so there's no way to instruct the monkey to sell then, but the second 6 occurs after the changes -1,-1,0,2.
So, if you gave the monkey that sequence of changes, it would wait until the first time it sees that sequence and
then immediately sell your hiding spot information at the current price, winning you 6 bananas.
...
Each buyer is going to generate 2000 secret numbers after their initial secret number, so, for each buyer, you'll have 2000 price changes in which your sequence can occur.
...
but for these four buyers, the sequence that will get you the most bananas is -2,1,-1,3. Using that sequence, the monkey will make the following sales:
...
def find_max_sequence(all_secrets: list[list[int]]) -> int:
    main_map = defaultdict(int)

    for seq in all_secrets:
        q = deque()
        seen = set()
        p = seq[0]
        for a in seq[1:]:
    ...
    with open(input_file) as file:
        for line in file.readlines():
            ...
            for _ in range(2000):
                ...
                seq.append(secret % 10)
            all_secrets.append(seq)

    if debug:
        print(f"Read {cnt} secrets. {len(all_secrets)=}, {len(all_secrets[0])=} ")

    return find_max_sequence(all_secrets)

def test_solve():
    assert solve("22_example.txt") == 23    

if __name__ == "__main__":
    test_solve()
    print(solve("22_input.txt"))
